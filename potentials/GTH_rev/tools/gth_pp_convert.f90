PROGRAM gth_pp_convert
  ...
  !          - Update of ABINIT output (11.08.2021, MK)
  ...
  WRITE (UNIT=unit_abinit,FMT="(A)")&
    "Goedecker-Teter-Hutter (GTH) pseudopotential for "//TRIM(elesym(iz))//&
    " (M. Krack, PSI)"
  WRITE (UNIT=unit_abinit,FMT="(I5,I4,2X,A,T83,A)")&
    iz,izeff,TRIM(date),"znucl zion pspdat"
  WRITE (UNIT=unit_abinit,FMT="(I5,I4,I3,I2,I5,I2,T83,A)")&
    10,xc_code_abinit,lppnl_max,MAX(0,2*(nppl-1)),2001,0,&
    "pspcod pspxc lmax lloc mmax r2well"
  line = " "
  WRITE (UNIT=line,FMT=fmtstr1)&
    rloc,nppl,(cppl(i),i=1,nppl)
  WRITE (UNIT=line(83:),FMT="(A,4(A2,I1))")&
    "rloc nloc",(" c",i,i=1,nppl)
  WRITE (UNIT=unit_abinit,FMT="(A)") TRIM(line)
  WRITE (UNIT=unit_abinit,FMT="(I5,T83,A)")&
    nppnl_max,"nnonloc"
  DO ippnl=1,nppnl_max
    line = " "
    WRITE (UNIT=line,FMT=fmtstr1)&
      rppnl(ippnl),nppnl(ippnl),(hppnl(1,j,ippnl),j=1,nppnl(ippnl))
    WRITE (UNIT=line(83:),FMT="(A,3(A3,I2))")&
    ...
    DO i=2,nppnl(ippnl)
      ...
      WRITE (UNIT=line,FMT=fmtstr2)&
        (hppnl(i,j,ippnl),j=i,nppnl(ippnl))
      WRITE (UNIT=line(83+5*i:),FMT="(3(A3,I2))")&
